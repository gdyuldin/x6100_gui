...
template <typename T> class ComputedParameter : public appcfg::SubjectT<T> {
  public:
    ...
    ComputedParameter(ComputeFn compute, ReverseFn reverse = {})
        : appcfg::SubjectT<T>(compute ? compute() : T{}), compute_(std::move(compute)), reverse_(std::move(reverse)) {}
