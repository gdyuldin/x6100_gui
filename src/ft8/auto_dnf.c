...
#define OVERLAY_X        13
#define OVERLAY_Y        13
#define OVERLAY_H        325

static auto_dnf_ctx_t *s_dnf_ctx            = NULL;
static lv_obj_t       *s_marker_waterfall   = NULL;
static uint64_t        s_worker_slot_id     = UINT64_MAX;
static uint64_t        s_waterfall_slot_id  = UINT64_MAX;

typedef struct {
    uint64_t slot_id;
} marker_msg_t;

static void marker_cb(void *arg) {
    const marker_msg_t *m = (const marker_msg_t *)arg;
    if (!s_marker_waterfall || !m) return;
    if (m->slot_id == s_waterfall_slot_id) return;
    s_waterfall_slot_id = m->slot_id;
    lv_waterfall_add_marker_line(s_marker_waterfall, lv_color_hex(0xFF0000));
}

static void schedule_slot_marker(const slot_info_t *info) {
    if (!info || !s_marker_waterfall) return;

    int      proto   = subject_get_int(cfg.ft8_protocol.val);
    uint64_t slot_ns = (proto == FTX_PROTOCOL_FT4) ? 7500000000ULL : 15000000000ULL;
    uint64_t slot_id = ((uint64_t)info->slot_start * 1000000000ULL) / slot_ns;

    if (slot_id == s_worker_slot_id) return;
    s_worker_slot_id = slot_id;

    marker_msg_t m = { .slot_id = slot_id };
    scheduler_put(marker_cb, &m, sizeof(m));
}
...
void ft8_autodnf_on_init(lv_obj_t *overlay_parent) {
    ...
    ft8_params_load(&tuning);
    s_dnf_ctx = auto_dnf_create(&tuning);
    auto_dnf_snapshot_entry(s_dnf_ctx);
    s_worker_slot_id    = UINT64_MAX;
    s_waterfall_slot_id = UINT64_MAX;

    if (overlay_parent && s_dnf_ctx) {
        int filt_low = 0, filt_high = 0;
        ft8_get_filter_range(&filt_low, &filt_high);
        /* Parent to waterfall (not dialog.obj) so peak lines sit above the PSD
         * but below the decode table sibling. Coords are relative to parent. */
        lv_obj_t *parent = s_marker_waterfall ? s_marker_waterfall : overlay_parent;
        lv_coord_t ox    = s_marker_waterfall ? 0 : OVERLAY_X;
        lv_coord_t oy    = s_marker_waterfall ? 0 : OVERLAY_Y;
        /* Frequency→pixel mapping must match waterfall width (771 in dialog_ft8),
         * not dialog.obj (796). */
        lv_coord_t overlay_w = 771;
        if (s_marker_waterfall) {
            lv_coord_t wf_w = lv_obj_get_width(s_marker_waterfall);
            if (wf_w > 0) overlay_w = wf_w;
        }
        auto_dnf_build_overlay(s_dnf_ctx, parent,
                               ox, oy, OVERLAY_H, overlay_w,
                               filt_low, filt_high);
    }
}

void ft8_autodnf_set_waterfall(lv_obj_t *waterfall_obj) {
    s_marker_waterfall = waterfall_obj;
}

void ft8_autodnf_on_cleanup(void) {
    if (!s_dnf_ctx) return;
    auto_dnf_restore_entry(s_dnf_ctx);
    auto_dnf_destroy(s_dnf_ctx);
    s_dnf_ctx            = NULL;
    s_marker_waterfall   = NULL;
    s_worker_slot_id     = UINT64_MAX;
    s_waterfall_slot_id  = UINT64_MAX;
}
...
void ft8_autodnf_on_psd(const float *psd, uint16_t nfft,
    ...
    auto_dnf_on_psd(s_dnf_ctx, psd, nfft, DNF_WORKER_RATE,
                    filt_low, filt_high, frame_ts,
                    ft8_is_our_tx_slot(info));
}
...
void ft8_autodnf_restore_entry(void) {
    if (s_dnf_ctx) auto_dnf_restore_entry(s_dnf_ctx);
}
