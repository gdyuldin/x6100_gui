...
public:
    static std::optional<int32_t> peek_int(const PendingWrites& pending,
    ...
    static std::optional<float> peek_float(const PendingWrites& pending,
                                           const StorageKey& key) {
        std::lock_guard<std::mutex> lock(pending.mutex_);
    ...
    static std::optional<std::string> peek_text(const PendingWrites& pending,
                                                const StorageKey& key) {
        std::lock_guard<std::mutex> lock(pending.mutex_);
