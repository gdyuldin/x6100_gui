...
TEST_CASE("DigitalModesTable find_next returns the next preset", "[digital_modes]") {
    ...
    DigitalModesTable::LoadResult res = DigitalModesTable::find_next(CFG_DIG_TYPE_FT8, 1'840'000);
...
TEST_CASE("DigitalModesTable find_next returns NOT_FOUND above the last preset", "[digital_modes]") {
    ...
    DigitalModesTable::LoadResult res = DigitalModesTable::find_next(CFG_DIG_TYPE_FT8, 52'000'000);
    REQUIRE(res.rc == NOT_FOUND);

    // Also NOT_FOUND when the current frequency equals the top preset (the
    // query uses a strict >).
    res = DigitalModesTable::find_next(CFG_DIG_TYPE_FT8, 50'313'000);
    REQUIRE(res.rc == NOT_FOUND);
}
...
TEST_CASE("DigitalModesTable find_prev returns the previous preset", "[digital_modes]") {
    ...
    DigitalModesTable::LoadResult res = DigitalModesTable::find_prev(CFG_DIG_TYPE_FT8, 14'074'000);
    REQUIRE(res.rc == SUCCESS);
...
TEST_CASE("DigitalModesTable find_prev returns NOT_FOUND below the first preset", "[digital_modes]") {
    ...
    DigitalModesTable::LoadResult res = DigitalModesTable::find_prev(CFG_DIG_TYPE_FT8, 1'840'000);
    REQUIRE(res.rc == NOT_FOUND);
}
...
TEST_CASE("DigitalModesTable find_closest selects the nearest preset", "[digital_modes]") {
    ...
    DigitalModesTable::LoadResult res = DigitalModesTable::find_closest(CFG_DIG_TYPE_FT8, 2'500'000);
    REQUIRE(res.rc == SUCCESS);
    REQUIRE(res.value.freq == 1'840'000);

    // 3.0 MHz is closer to 80m (3.573 MHz, |d|=573k) than to 160m (1.84 MHz).
    res = DigitalModesTable::find_closest(CFG_DIG_TYPE_FT8, 3'000'000);
    REQUIRE(res.rc == SUCCESS);
    REQUIRE(res.value.freq == 3'573'000);
}
...
TEST_CASE("DigitalModesTable isolates FT8 and FT4 presets", "[digital_modes]") {
    ...
    DigitalModesTable::LoadResult res = DigitalModesTable::find_next(CFG_DIG_TYPE_FT8, 3'573'000);
    REQUIRE(res.rc == SUCCESS);
    REQUIRE(res.value.freq == 5'357'000);
    REQUIRE(res.value.label == "FT8 60m");

    // Same boundary from the prev direction.
    res = DigitalModesTable::find_prev(CFG_DIG_TYPE_FT8, 5'357'000);
    REQUIRE(res.rc == SUCCESS);
    REQUIRE(res.value.freq == 3'573'000);

    // FT4 navigation still finds its own rows interleaved with the FT8 ones.
    res = DigitalModesTable::find_next(CFG_DIG_TYPE_FT4, 3'575'000);
    REQUIRE(res.rc == SUCCESS);
...
TEST_CASE("DigitalModesTable returns NOT_FOUND for an empty table", "[digital_modes]") {
    ...
    REQUIRE(DigitalModesTable::find_next(CFG_DIG_TYPE_FT8, 1'840'000).rc == NOT_FOUND);
    REQUIRE(DigitalModesTable::find_closest(CFG_DIG_TYPE_FT8, 7'074'000).rc == NOT_FOUND);
    REQUIRE(DigitalModesTable::find_prev(CFG_DIG_TYPE_FT8, 7'074'000).rc == NOT_FOUND);
}
...
TEST_CASE("DigitalModesTable navigates up/down/up in sequence", "[digital_modes]") {
    ...
    res = DigitalModesTable::find_next(CFG_DIG_TYPE_FT8, res.value.freq);
    REQUIRE(res.rc == SUCCESS);
    REQUIRE(res.value.freq == 5'357'000);

    // One step back down.
    res = DigitalModesTable::find_prev(CFG_DIG_TYPE_FT8, res.value.freq);
    REQUIRE(res.rc == SUCCESS);
    REQUIRE(res.value.freq == 3'573'000);

    // Closest from the returned preset lands back on itself.
    res = DigitalModesTable::find_closest(CFG_DIG_TYPE_FT8, res.value.freq);
    REQUIRE(res.rc == SUCCESS);
    REQUIRE(res.value.freq == 3'573'000);
}
...
TEST_CASE("DigitalModesTable stores the preset label", "[digital_modes]") {
    ...
    DigitalModesTable::LoadResult res = DigitalModesTable::find_prev(CFG_DIG_TYPE_FT8, 5'357'000);
    REQUIRE(res.rc == SUCCESS);
    REQUIRE(res.value.label == "FT8 80m");
}
...
TEST_CASE("DigitalModesTable loads the mode column", "[digital_modes]") {
    ...
    DigitalModesTable::LoadResult res = DigitalModesTable::find_next(CFG_DIG_TYPE_FT8, 1'000'000);
    REQUIRE(res.rc == SUCCESS);
    REQUIRE(res.value.freq == 1'840'000);
